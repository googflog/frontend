...
class TrackGA {
  ...
  setEvent() {
    $(this.$target).on('click.trackGA', (e) => {
      ...
      f.value    = this.feeds.value    || $target.attr('data-value');
  ...
  static send(feeds) {
    ...
    if (f.category !== undefined) { d.eventCategory = f.category; }
    if (f.action   !== undefined) { d.eventAction   = f.action; }
    if (f.label    !== undefined) { d.eventLabel    = f.label; }
    if (f.value    !== undefined) { d.eventValue    = f.value; }
